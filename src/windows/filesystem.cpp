...
AL2O3_EXTERN_C int Os_SystemRun(char const* fileName, int argc, const char** argv) {
	...
	if (!CreateProcessA(
		NULL, (LPSTR)commandLine.c_str(), NULL, NULL, FALSE, 0, NULL, NULL, &startupInfo, &processInfo))
		return -1;
...
LOGWARNING("Os_DirectoryEnumeratorAsyncStart isn't async on windows yet, will be a busy sync for now");
